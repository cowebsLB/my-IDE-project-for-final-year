...
ipcMain.handle("fs-read-file", async (_event, filePath) => {
  try {
    const content = await fs$4.readFile(filePath, "utf-8");
...
ipcMain.handle("fs-write-file", async (_event, filePath, content) => {
  try {
    await fs$4.writeFile(filePath, content, "utf-8");
    return { success: true };
  } catch (error) {
...
ipcMain.handle("fs-read-directory", async (_event, dirPath) => {
  try {
    const entries = await fs$4.readdir(dirPath, { withFileTypes: true });
...
ipcMain.handle("dialog-save-file", async (_event, defaultFilename) => {
  if (!win) return null;
...
ipcMain.handle("watch-directory", async (_event, dirPath) => {
  ...
  currentWatcher.on("all", (event, path2) => {
    win == null ? void 0 : win.webContents.send("fs-change", { type: event, path: path2 });
  });
  return true;
});
app.whenReady().then(() => {
